...
function qrImg(data, size = 200) {
  // Public QR image service; the raw link is always shown as a fallback.
...
export function hotelDetailPage(hotel, stats, origin) {
  ...
  <div class="grid">
    ...
    <section class="card">
      ...
      <div class="qr">
        <img src="${esc(qrImg(reviewUrl))}" alt="QR code linking to the review page" width="200" height="200" />
...
export function promotePage(hotel, stars) {
  ...
  if (!buttons.length) {
    buttons.push(`<p class="muted">The hotel hasn't added public review links yet.</p>`);
  }
  const body = `
  <div class="guest">
    <div class="big-stars">${'★'.repeat(stars)}${'☆'.repeat(5 - stars)}</div>
    <h1>Wonderful — thank you!</h1>
    <p class="lead">Would you share your experience publicly? It only takes a moment and means the world to us.</p>
